...
import { pages as pagess } from "../Pages/Pages";
        ...
        <Link
            ...
            onClick={(e) => {
                ...
                if (location && windowRef.current && href !== "") {
                    const hrefFormat = href?.startsWith("/")
                        ? href
                        : `${location.pathname.startsWith("/") ? location.pathname.slice(1) : location.url.pathname}/${href}`;
                    windowRef.current.history.pushState(
                        {},
                        location.title,
                        location.url,
                    );
                    windowRef.current.history.replaceState(
                        {},
                        title ?? "",
                        hrefFormat,
                    );
                    if (location?.scrollToPage)
                        location.scrollToPage(
                            pages.includes(hrefFormat)
                                ? pages.findIndex(
                                      (page) => page === hrefFormat,
                                  ) + 1
                                : 0,
                        );
